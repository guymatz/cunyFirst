...
parser = argparse.ArgumentParser(
    description='Check availability of CUNY classes in CUNYFirst')
...
def logout(browser):
    if args.debug:
        print("Logging out")
    browser.open(
        'https://home.cunyfirst.cuny.edu/psp/cnyepprd/EMPLOYEE/EMPL/?cmd=logout')
...
if args.debug:
    print("Submitting search page . . .")
br.submit()
writeout("past_search_form", br)

if args.debug:
    print("Submitting search page again . . !")
try:
    br.select_form(nr=0)
    mods = br.find_control('ICAction')
    mods.readonly = False
